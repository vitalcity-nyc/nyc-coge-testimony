...
WAV="transcripts/audio/queens.wav"
...
json.dump(segs, open("transcripts/queens.json","w"), ensure_ascii=False, indent=1)
...
for s in segs:
    ...
    cur.append(s["text"])
    if s["start"]-cs>=20:
        blocks.append((cs," ".join(cur))); cur=[]; cs=None
if cur: blocks.append((cs," ".join(cur)))
with open("transcripts/queens.txt","w") as f:
    for st,txt in blocks: f.write(f"[{mmss(st)} | t={int(st)}] {txt}\n")
print(f"wrote queens.txt: {len(blocks)} blocks, last t={int(segs[-1]['start'])}s ({mmss(segs[-1]['start'])})")
