...
**Both watchers are RETIRED as of 2026-07-27.** The commission is finished — ten hearings held, all ten ingested and all ten reconciled against COGE's own transcripts, final meeting July 23 — so there is nothing left for a daily job to find. Nothing about this project runs on a schedule any more; treat it as a finished, static catalog.

- **Scheduled task `coge-testimony-watch`** (`~/.claude/scheduled-tasks/coge-testimony-watch/SKILL.md`) ran daily ~10am and did the whole loop: detect new videos, ingest, reconcile minutes, verify the deploy, and iMessage Josh (**9175823254**) only when something changed. It is now **disabled** (not deleted — the SKILL.md is intact). If a 2027 commission ever needs the same treatment, re-enable it with the scheduled-tasks tools and update the borough/round list in the SKILL.md; the ingest pipeline below is the reusable part.
- **The old launchd watcher (`com.josh.coge-watcher`)** was booted out and its plist renamed to `~/Library/LaunchAgents/com.josh.coge-watcher.plist.retired` on 2026-07-27. It must stay off: `scripts/check_new_hearings.py` writes new video IDs into `known_videos.json` (line 83) *before* any ingest runs, so the ingest task then saw them as "known" and skipped them — this is why SI R2 + Manhattan R2 were silently missed once. Despite an earlier note claiming it was already disabled, it was in fact still firing at 09:15 daily right up to 2026-07-27. If it ever reappears: `launchctl bootout gui/$(id -u)/com.josh.coge-watcher`.
