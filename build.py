...
/* Embed mode (?embed=1): strip the nav and the oversized hero so the tool
   sits cleanly inside a Ghost article that already has its own headline.
   Keeps the kicker, byline, stats, scope box, the full tool, methodology
   and the AI-caution button. */
html.embed .topbar{display:none;}
html.embed .hero h1{display:none;}
...
/* secondary to the orange Gemini Notebook CTA just below it — two orange buttons
   stacked in a narrow article column read as one repeated call to action */
.embed-full{display:inline-block;background:var(--vc-white);color:var(--vc-black);
  font-family:var(--sans);font-weight:700;font-size:12px;letter-spacing:.08em;text-transform:uppercase;
  text-decoration:none;padding:10px 15px;border-radius:4px;border:2px solid var(--vc-black);
...
/* fixed-position inside an auto-height iframe would pin to the very bottom of the
   whole document; anchor it to the end of the content instead */
html.embed .ai-btn{position:absolute;}
html.embed .ai-pop{position:absolute;}
html.embed body{position:relative;}
</style>
</head>
<body>
<script>
// Embed mode (?embed=1): strip outer chrome and post content height to the parent
// so a Ghost HTML card can size its iframe to fit — no inner scrollbar, no
// trailing whitespace. This page is interactive (theme accordions, view toggle,
// filters all change height), so we re-measure on any body resize, not just load.
if(new URLSearchParams(location.search).get('embed')==='1'){
  document.documentElement.classList.add('embed');
  var __lastH=0;
  var postHeight=function(){
    // Measure the BODY box, not documentElement.scrollHeight: scrollHeight is
    // floored by the viewport, and inside an auto-sized iframe the viewport IS
    // whatever height the parent last set. Using it makes the height a ratchet
    // that can only grow — collapsing an accordion would never shrink the frame.
    var h=Math.ceil(document.body.getBoundingClientRect().height);
    if(!(h>0)) h=document.body.scrollHeight;
    if(Math.abs(h-__lastH)<2) return; __lastH=h;
    parent.postMessage({type:'vc-embed-height', id:'nyc-coge-testimony', height:h}, '*');
  };
  window.addEventListener('load', postHeight);
  window.addEventListener('resize', postHeight);
  document.addEventListener('DOMContentLoaded', function(){
    // catches accordion expand/collapse, view toggle, filtering, search
    if(window.ResizeObserver){ new ResizeObserver(postHeight).observe(document.body); }
    postHeight();
  });
  // fonts/late layout can shift height right after load
  var __n=0, __t=setInterval(function(){ postHeight(); if(++__n>20) clearInterval(__t); }, 300);
}
</script>
...
<header class="hero"><div class="wrap">
  ...
  <div class="cta-row">
    ...
    <span class="cta-note">Query the full testimony in plain language, via a Gemini Notebook built on the same hearing record.</span>
  </div>
...
<section class="method"><div class="wrap">
  ...
  <p>Some major interests are shaping COGE from inside rather than through testimony: DC 37's Henry Garrido and the Partnership for New York City's Kathryn Wylde sit on the commission itself. All ten borough hearings are now catalogued and the commission has finished its work. Written testimony is another gap: COGE publishes no archive of the comments it received, so a written submission appears here only when the witness or their organization published it themselves.</p>
